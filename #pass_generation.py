...
# and this is to make a password in one time not in a loop V;

import string
import random

def make_serial(count):
    all_chars = string.ascii_letters + string.digits  # All letters and digits
    serial = ''.join(random.choices(all_chars, k=count))  # Generate a random 30-character string at once
    print(serial)  # Print the result once

# In the function random.choices(population, k):
# population: The set of elements to choose from (e.g., all_chars containing letters and digits).
# k: The number of elements to randomly select from the population.

make_serial(30)  # Generate a 30-character random serial
